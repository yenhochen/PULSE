...
def downloadextract_HARfiles(
    zippath="data/har.zip", targetpath="data/har", redownload=False
):
...
def preprocess_HARdata(
    harpath="data/har", processedharpath="data/har/processed", reprocess=False
):
    ...
    for patient in patient_list:
        acc = (
            pd.read_csv(
                os.path.join(harpath, f"RawData/acc_{patient}"),
                delimiter=" ",
                header=None,
            )
            .to_numpy()
    ...
    for patient in patient_list:
        ...
        gyro = (
            pd.read_csv(
                os.path.join(harpath, f"RawData/gyro_{patient}"),
                delimiter=" ",
                header=None,
            )
            .to_numpy()
    ...
    labels = pd.read_csv(
        os.path.join(harpath, "RawData/labels.txt"), delimiter=" ", header=None
    )
    ...
    for i in range(labels.shape[0]):
        label_dict[(labels.iloc[i, 0], labels.iloc[i, 1])].append(
            (labels.iloc[i, 2], labels.iloc[i, 3], labels.iloc[i, 4])
        )
    ...
    def segment_subseqwithlabels(
        ...
        for idx, patientname in enumerate(patients):
            ...
            for labeltriplet_forexpuser in labels_forexpuser:
                ...
                while (
                    i + subseq_len < threshold_len
                    and i + subseq_len <= labeltriplet_forexpuser[2]
                ):
    ...
    train_data_subseq, train_labels_subseq, train_patients_subseq = (
        segment_subseqwithlabels(train_patients, train_data)
    )
    np.save(os.path.join(processedharpath, "train_data_subseq.npy"), train_data_subseq)
    np.save(
        os.path.join(processedharpath, "train_names_subseq.npy"), train_patients_subseq
    )
    np.save(
        os.path.join(processedharpath, "train_labels_subseq.npy"),
        train_labels_subseq - 1,
    )  # -1 so that labels start from 0

    val_data_subseq, val_labels_subseq, val_patients_subseq = segment_subseqwithlabels(
        val_patients, val_data
    )
    ...
    np.save(
        os.path.join(processedharpath, "val_labels_subseq.npy"), val_labels_subseq - 1
    )

    test_data_subseq, test_labels_subseq, test_patients_subseq = (
        segment_subseqwithlabels(test_patients, test_data)
    )
    np.save(os.path.join(processedharpath, "test_data_subseq.npy"), test_data_subseq)
    np.save(
        os.path.join(processedharpath, "test_names_subseq.npy"), test_patients_subseq
    )
    np.save(
        os.path.join(processedharpath, "test_labels_subseq.npy"), test_labels_subseq - 1
    )
    ...
    write_json(label_name, os.path.join(processedharpath, "label_name.json"))
